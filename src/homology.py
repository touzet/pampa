...
def find_markers_single_sequence(seq, set_of_digested_peptides, dict_of_model_markers, set_of_markers, taxo):
    ...
    # dict_of_model_markers, dictionary, key: peptide sequence [string], value: set of 3-uplets (PTM, code, gene name)
    ...
    found_markers={} # key: name of the marker, value: 4-uplet (position, hamming distance, Pmask, marker sequence)
    ...
    for marker_seq in dict_of_model_markers.keys():      
        ...
        if seq.protein() is not None and seq.protein() not in set_of_protein_names:
            continue
        set_of_codes={s[1]  for s in dict_of_model_markers[marker_seq]}
    ...
    for code in found_markers:
        ...
        for model_marker in dict_of_model_markers[marker_seq]:
            ...
            if helical_start is not None and pos>helical_start:
                dict_marker["Hel"]=pos-helical_start+2
            if new_sequence not in set_of_raw_digested_peptides:
                dict_marker["Digestion"]="No"
            else:
                dict_marker["Digestion"]="Yes"
            if d==0:
                dict_marker["Comment"]="Homology : "+ seq.sequence()[pos-1]+" - peptide - "+ seq.sequence()[pos+l]+", exact match"
            elif d==1:
                dict_marker["Comment"]="Homology : "+ seq.sequence()[pos-1]+" - peptide - "+ seq.sequence()[pos+l]+ ", 1 mismatch with " + marker_seq
            else:
                dict_marker["Comment"]="Homology : "+ seq.sequence()[pos-1]+" - peptide - "+ seq.sequence()[pos+l]+ ", "+str(d)+ " mismatches with " + marker_seq
            if taxo:
                set_of_potential_taxids={m.taxid() for m in set_of_markers if m.sequence()==marker_seq}
                proximity=taxonomy.find_closest_ID(seq.taxid(),set_of_potential_taxids,taxo)
                dict_marker["Comment"]=dict_marker["Comment"]+proximity
            dict_marker["Comment"]=dict_marker["Comment"]+". "
            if collagen.P_pattern(new_sequence)[0]==collagen.P_pattern(marker_seq)[0]:
                dict_marker["PTM"]=model_marker[0]
            else:
                dict_marker["Comment"]=dict_marker["Comment"]+ "PTM updated. "
...
def find_markers_all_sequences(set_of_sequences, set_of_markers, taxo, config_digestion):
    ...
    for m in set_of_markers: 
        ...
        if m.code() is None or len(m.code())==0 :
            marker_count+=1
            peptide_name="M"+str(marker_count) # TO MODIFY
        else:
            peptide_name=m.code()
    ...
    min_length=min(len(peptide) for peptide in dict_of_model_markers)
    max_length=max(len(sequence) for sequence in dict_of_model_markers)
    set_of_new_markers=set()
    for seq in set_of_sequences:
        set_of_digested_peptides=sequences.in_silico_digestion({seq}, config_digestion,min_length,max_length)
