...
def main():
    ...
    parser.add_argument("--placentals", dest="placentals", action='store_true', help="Use placental model", required=False)
    ...
    try:
        ...
        (homology, deamidation, allpeptides, fillin, selection, reconstruction, peptide_table, fasta, fasta_dir, spectra, error, limit, taxonomy, config, placentals, target) = params_checker.check_and_update_parameters_craft(args.homology, args.deamidation, args.allpeptides, args.fillin, args.selection, args.reconstruction, args.peptide_table, args.fasta, args.fasta_dir, args.spectra, args.resolution, args.limit, args.taxonomy, args.config, args.placentals, args.target)
        
        list_of_constraints = lmt.parse_limits(limit)
        full_taxonomy = ta.parse_taxonomy_simple_file(taxonomy)
        ta.table_print(full_taxonomy)
        ...
        if homology:
            ...
            list_of_new_markers=ta.add_taxonomy_ranks(list_of_new_markers, full_taxonomy)
            pt.build_peptide_table_from_set_of_markers(list_of_new_markers,output, config_headers, config_markers)
            rep.create_report_homology(peptide_table, set_of_markers, list_of_new_markers, fasta, fasta_dir, set_of_sequences, taxonomy, config_digestion, limit, list_of_constraints, web)
            
        if deamidation:
            set_of_codes=lmt.deamidated_codes(list_of_constraints)
        ...
        if fillin:
            ...
            set_of_new_markers=ta.supplement_taxonomic_information(set_of_new_markers, full_taxonomy)
            ...
            set_of_new_markers=ta.add_taxonomy_ranks(list_of_markers, full_taxonomy)
